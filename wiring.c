...
#define _MORENOP_  ""
#define _ONE_NOP_  " nop \n\t "
#define _TWO_NOP_   _ONE_NOP_  _ONE_NOP_
#define _FOUR_NOP_  _TWO_NOP_  _TWO_NOP_
#define _FIVE_NOP_  _FOUR_NOP_ _ONE_NOP_
#define _SIX_NOP_   _FOUR_NOP_ _TWO_NOP_
...
// # elif F_CPU >= 29491200L
  ...
  // the following loop takes a 2/5 of a microsecond (10 cycles)
  // per iteration, so execute it five times for every two microseconds of
  // delay requested.
  us = (us << 1) + (us >> 1); // x2.5, = 7 cycles

  // make loop below take 10 cycles
#undef  _MORENOP_
#define _MORENOP_ _SIX_NOP_

  // account for the time taken in the preceeding commands.
  // we just burned 22 (24) cycles above, remove 2 (2*10=20)
  // us is at least 2 so we can only subtract 1
  us -= 1; // = 2 cycles
  ...
  // make loop take six cycles for 1/4th of a microsecond
#undef  _MORENOP_
#define _MORENOP_ _TWO_NOP_

  // account for the time taken in the preceeding commands.
  // we burn 19 (21) cycles above plus 2 below, remove 3, (3*6=18)
...
#elif F_CPU >= 20000000L
  __asm__ __volatile__ ( _FIVE_NOP_ ); // just waiting 5 cycles

  // for a one-microsecond delay, simply return.  the overhead
  // of the function call takes 19 (21) cycles, which is 1us
  if (us <= 1) return; //  = 3 cycles, (4 when true)

  // make loop slower by 5/4 such that we must only multiply by 4
#undef  _MORENOP_
#define _MORENOP_ _ONE_NOP_
  ...
  // we burn 24 (26) cycles above plus 2 below, remove 5 (5*5=25)
